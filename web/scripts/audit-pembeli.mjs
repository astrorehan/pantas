...
async function runAudit(isMobile = false) {
  ...
  await page.evaluate(
    ...
    STORAGE_KEY,
    JSON.stringify({ sesi: SESI_PEMBELI }),
  );
  await page.close();
  ...
  const runnerResult = await lighthouse("http://localhost:3000/pembeli", flags, config);
  const report = runnerResult.lhr;

  console.log(`\n========================================`);
  console.log(`LIGHTHOUSE AUDIT: /pembeli (${isMobile ? "MOBILE" : "DESKTOP"})`);
  console.log(`========================================`);
...
async function main() {
  console.log("Starting Mobile Audit for /pembeli...");
  await runAudit(true);
  console.log("\nStarting Desktop Audit for /pembeli...");
